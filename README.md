...
3. 使用python构建，借助 `pymongo模块` 实现数据导出。服务器或者电脑自带py环境，可以直接运行导出现有数据甚至部署给服务器或本地电脑
4. 配置简单：仅需填写仓库的`secret`，避免小白改源码发生其它错误

## 前言

Hexo博客与其它框架不同，它是前后端分离的，上个月在博客群看到了恶意刷评论的站点，这对于Hexo是致命的，解决办法就是勤备份，所以在八月初我就向作者提出了建议并附上方案[Issue #429 · imaegoo/twikoo ](https://github.com/imaegoo/twikoo/issues/429)，原本想整理发出来，但一拖再拖，方案也迭代了三次做了一些优化
...
![Twikoo评论定时备份方案05](https://test1.jsdelivr.net/gh/GC-ZF/Comment-Backups/img/Twikoo评论定时备份方案05.png)
...
![Twikoo评论定时备份方案06](https://test1.jsdelivr.net/gh/GC-ZF/Comment-Backups/img/Twikoo评论定时备份方案06.png)

选择MongoDB Compass连接，安装软件

![Twikoo评论定时备份方案07](https://test1.jsdelivr.net/gh/GC-ZF/Comment-Backups/img/Twikoo评论定时备份方案07.png)

切记将"\<password>"替换成自己的数据库密码，再在软件中输入连接字符串

![Twikoo评论定时备份方案08](https://test1.jsdelivr.net/gh/GC-ZF/Comment-Backups/img/Twikoo评论定时备份方案08.png)

**『MongoDBCompass』**可以帮助我们对评论增删改查，这样就拥有了类似tyepcho的评论后端，简单使用图解：

![Twikoo评论定时备份方案09](https://test1.jsdelivr.net/gh/GC-ZF/Comment-Backups/img/Twikoo评论定时备份方案09.png)
